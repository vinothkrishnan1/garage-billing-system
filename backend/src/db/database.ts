  ...
  if (productCount && productCount.count === 0) {
    ...
    for (const prod of sampleProducts) {
      await dbRun(
        ...
        [prod.name, prod.stock_qty, prod.selling_price]
      );
    }
  }

  const customerCount = await dbGet<{ count: number }>('SELECT COUNT(*) as count FROM customers');
  if (customerCount && customerCount.count === 0) {
    ...
    for (const cust of sampleCustomers) {
      await dbRun(
        ...
        [cust.vehicle_number, cust.vehicle_model, cust.km_driven, cust.customer_name, cust.mobile_number]
      );
    }
    ...
    const cust = await dbGet<{ id: number }>('SELECT id FROM customers WHERE vehicle_number = ?', [sampleBill.vehicle_number]);
    ...
    for (const item of items) {
      await dbRun(
        ...
        [billId, item.s_no, item.product_name, item.qty, item.amount]
      );
    }
  }
}
